...
import { archiveDate, getViewRoleNonOperatorIds, getViewRoleOperatorIds } from "./helpers/helpers";
import { handlePartnerStatusChange } from "./helpers/helpers";
import { setIsHidden } from "./helpers/styleHelpers";
...
export default function AFE() {
  ...
  afes?.sort((a,b) => b.sortID - a.sortID)
  ...
  return (
    ...
    <div hidden = {currentTab ===2} className="py-4 px-4 sm:px-8">
      ...
      {nonOperatedAFEs?.map((afe) => (
        ...
        onClick={ (e:any) =>{handlePartnerStatusChange(`${afe.id}`, `${afe.partner_status}`,afe.partner_status === 'New' ? 'Viewed' : `${afe.partner_status}`, 'The Partner Status on the AFE changed from New to Viewed','action'), refreshData()}}
