...
export default function GLLibrary() {
  ...
  const [accountCodes, setAccountCodes] = useState<GLCodeType[] | []>([]);
  const [partnerAPCID, setPartnerAPCID] = useState<string>('');
  const [hideDeletedAccounts, setHideDeletedAccounts] = useState<boolean>(true);
  const [loading, setLoading] = useState(false);

  const handlePageChange = (paginatedData: GLCodeType[], page: number) => {
    ...
    <>
      <div className="rounded-lg bg-white shadow-2xl ring-1 ring-[var(--darkest-teal)]/70 p-4 mb-5">
        <div className="grid grid-cols-1 gap-x-8 gap-y-10 sm:grid-cols-3 sm:divide-x sm:divide-[var(--darkest-teal)]/40">
          <div className="">
            <h2 className="text-base/7 font-semibold text-[var(--darkest-teal)] custom-style">View the GL Account Codes for Operated or Non-Op AFEs</h2>
          ...
          <div className="col-span-2 grid grid-cols-1 gap-x-8 gap-y-10 ">
            <div className="flex flex-col lg:flex-row">
              ...
              <div className='grow-2 m-2'>
                <h1 className="text-base/7 font-medium text-[var(--darkest-teal)] custom-style">Select the Operator for Non-Op AFEs</h1>
                <PartnerDropdown
                value={partnerAPCID}
      ...
      <div hidden={opAPCID === '' && partnerAPCID === '' ? true : false } 
      className="mt-5 bg-white">
        <table className="table-auto min-w-full ring-1 ring-[var(--darkest-teal)]/70 rounded-t-md">
          ...
          <tbody className="border-b border-b-[var(--darkest-teal)]/30">
            {rowsToShow.map((accountRow, accountIdx) => (
              <tr key={accountIdx} 
              hidden={hideDeletedAccounts && !accountRow.active ? true : false}
              className="text-end sm:text-start custom-style-long-text border-l border-l-[var(--darkest-teal)]/30 even:bg-[var(--darkest-teal)]/20">
                ...
                <td scope="col" className="px-2 border-r border-r-[var(--darkest-teal)]/30">
                  <div hidden={accountRow.active ? false : true}
                    className="m-1 flex items-center justify-center pt-1 group relative">
                    <button
                      onClick={e => handleDeleteAccount(accountRow.id, !accountRow.active)}
                  ...
                  <div hidden={accountRow.active ? true : false}
                    className="m-1 flex items-center justify-center pt-1 group relative">
                    <button
                      onClick={e => handleDeleteAccount(accountRow.id, !accountRow.active)}
        ...
        <div className="pt-6"
        hidden={accountCodes.length===0 ? true : false}>
        <UniversalPagination
          data={accountCodes}
        ...
        <div className="flex flex-row-reverse justify-start gap-3 py-6 px-0">
          <SingleCheckbox
            value={hideDeletedAccounts}
            onChange={(hideDeletedAccounts) => setHideDeletedAccounts(hideDeletedAccounts)}
            label={'Hide Deleted Account Codes'}
            id={'hideDeletedAccounCodes'}>
          </SingleCheckbox>
        </div>
      </div>
