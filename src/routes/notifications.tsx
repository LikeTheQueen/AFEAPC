import { fetchAFENotificationCount, fetchNotifications } from "provider/fetch";
...
useEffect(() => {
    ...
    async function getNotifications() {
        try {
            const result = await fetchNotifications(minRange, maxRange);
...
useMemo(() => {
  async function getNotificationHistoryRowCount() {
    const systemHistoryRowCountResult = await fetchAFENotificationCount();
    setTotalNotificationHistoryRowCount(systemHistoryRowCountResult);
...
const filterNotifications = (notificationsList: Notifications[]) => {
    return notificationsList.filter(notice => {
      ...
      return matchesUser && matchesAFENumber && matchesDescription
    })
  };
